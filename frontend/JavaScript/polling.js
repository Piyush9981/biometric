            ...
            data.requests.forEach(req => {
                ...
                html += `
                    <tr data-status="${req.request_status}">
                        ...
                        <td>${displayExit}</td>
                        <td>${displayEntry}</td>
                        <td>${displayActualExit}</td>
                        <td>${displayActualEntry}</td>
                        <td>
                        ...
                        <td><span class="status-badge ${statusClass}">${req.request_status}</span></td>
                        <td>${actionsHtml}</td>
                    </tr>
                `;
            });
...
async function updateGatekeeperDashboard() {
    try {
        ...
        if (data.requests.length === 0) {
            html = '<tr><td colspan="10" class="text-center">No requests found.</td></tr>';
        } else {
            data.requests.forEach(req => {
                const displayExit = convertUTCToIST(req.requested_exit_datetime);
                const displayEntry = convertUTCToIST(req.requested_entry_datetime);
                ...
                html += `
                    <tr data-status="${req.request_status}">
                        <td>${req.student_id}</td>
                        <td>
                            <div class="student-profile-container" style="display: none;">
                                ${profileHtml}
                            </div>
                            ${req.student_name}
                        </td>
