...
document.addEventListener('DOMContentLoaded', () => {
                        ...
                        if (historyTableBody) {
                            ...
                            if (data.history.length === 0) {
                                ...
                                data.history.forEach(req => {
                                    ...
                                    row.innerHTML = `
                                        <td><span class="td-val">${convertUTCToIST(req.requested_exit_datetime)}</span></td>
                                        <td><span class="td-val text-right">${req.outing_reason}</span></td>
                                        <td><span class="td-val text-right">${req.destination}</span></td>
                                        <td><span class="td-val"><span class="status-badge ${req.request_status.toLowerCase().replace(/\s+/g, '-')}">${req.request_status}</span></span></td>
                                        <td><span class="td-val text-right">${req.warden_remarks || '-'}</span></td>
                                    `;
    ...
    if (table && colToggles.length > 0) {
        ...
        const savedCols = JSON.parse(localStorage.getItem('outpass_dashboard_column_prefs')) || {};
        ...
        colToggles.forEach(toggle => {
            ...
            toggle.addEventListener('change', (e) => {
                ...
                localStorage.setItem('outpass_dashboard_column_prefs', JSON.stringify(savedCols));
            });
        });
    }
    ...
    function renderHistoryTable(data, prefix) {
        ...
        data.history.forEach(item => {
            ...
            tr.innerHTML = `
                <td><span class="td-val">${datePart}</span></td>
                <td>
                ...
                <td><span class="td-val"><span class="status-badge ${statusClass}">${item.request_status}</span></span></td>
                <td><span class="td-val text-right">${item.warden_remarks || '-'}</span></td>
            `;
